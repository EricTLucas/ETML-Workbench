...
5. Trains the default sklearn linear and random-forest candidates, plus an automatic dummy baseline. Selects the winner using validation macro F1.
...
```bash
python -m pip install -e ".[ui,huggingface]"
workbench ui --workspace datasets
```

The local Streamlit interface defaults to port 8501 and binds to `127.0.0.1`. Use `--port 8502` for another port or `--headless` to avoid automatic browser opening.

The pages are:

1. **Import**: upload a file or separate train/test/validation files, or choose a supported provider.
2. **Prepare**: select target/type/exclusions and prepare a task, optionally using an approved recipe.
3. **Train**: choose models and settings; view validation comparisons and timings.
4. **Predict one**: fill in raw values for the selected model and inspect its prediction/probabilities.
5. **Reproduce**: inspect setup information and export the replay package.

Use the CLI for detailed recipe revision, custom search spaces, advanced split options, or other controls not exposed by the current interface. The UI is a local workbench, not a hosted multi-user application or production prediction API.
...
| `ui` | Launch local interface; options include `--workspace`, `--port`, `--headless` |
